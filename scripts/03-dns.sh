...
#	nslookup biblioteca.itauna.mg.gov.intra (query Internet name servers interactively)
#	dig biblioteca.itauna.mg.gov.intra (DNS lookup utility)
#	host biblioteca.itauna.mg.gov.intra (DNS lookup utility)
#	ping biblioteca.itauna.mg.gov.intra (send ICMP ECHO_REQUEST to network hosts)
#
# Windows Powershell.: 
...
#	ipconfig /displaydns
#	ping biblioteca.itauna.mg.gov.intra
#	Resolve-DnsName biblioteca.itauna.mg.gov.intra
#	Test-Connection biblioteca.itauna.mg.gov.intra
#
...
sleep 5
#
echo -e "Editando o arquivo de configuração biblioteca.itauna.mg.gov.intra.hosts, pressione <Enter> para continuar."
	# opção do comando: &>> (redirecionar a saída padrão)
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim /var/lib/bind/biblioteca.itauna.mg.gov.intra.hosts
	named-checkzone $DOMAIN /var/lib/bind/biblioteca.itauna.mg.gov.intra.hosts &>> $LOG
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
